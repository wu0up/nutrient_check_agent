#Codes from https://github.com/andrewyng/translation-agent
#modified to support llamaindex and Easy modify to access to other APIs
...
#base_url = os.environ.get("BASE_URL") if "BASE_URL" in os.environ else None
groq_model = os.environ.get("MODEL")
...
def one_chunk_initial_translation(source_lang: str, target_lang: str,
                                  source_text: str) -> str:
    """
...
def one_chunk_translate_text(source_lang: str,
                             target_lang: str,
                             source_text: str,
                             country: str = "") -> str:
    """
    ...
    reflection = one_chunk_reflect_on_translation(source_lang, target_lang,
...
def num_tokens_in_string(input_str: str,
...
def multichunk_initial_translation(source_lang: str, target_lang: str,
                                   source_text_chunks: List[str]) -> List[str]:
    """
...
"""
    ...
    for i in range(len(source_text_chunks)):
        ...
        tagged_text = ("".join(source_text_chunks[0:i]) + "<TRANSLATE_THIS>" +
                       source_text_chunks[i] + "</TRANSLATE_THIS>" +
...
def multichunk_translation(source_lang,
                           target_lang,
                           source_text_chunks,
                           country: str = ""):
    """
    ...
    return translation_1_chunks, reflection_chunks, translation_2_chunks
...
def translate(
    ...
    if num_tokens_in_text < max_tokens:
        ...
        init_translation, reflect_translation, final_translation = one_chunk_translate_text(
        ...
        token_size = calculate_chunk_size(token_count=num_tokens_in_text,
                                          token_limit=max_tokens)

        ic(token_size)

        text_parser = SentenceSplitter(chunk_size=token_size, )

        source_text_chunks = text_parser.split_text(source_text)

        ic(source_text_chunks)

        translation_1_chunks, reflection_chunks, translation_2_chunks = multichunk_translation(
            source_lang, target_lang, source_text_chunks, country)

        init_translation = "".join(translation_1_chunks)
        reflect_translation = "".join(reflection_chunks)
